<template>
  <div  class="bg-cover bg-gradient-to-r from-lime-500 via-lime-800 to-lime-700 ">
    <div>
        <h1 class="text-6xl font-normal leading-normal mt-0 mb-2 text-white font-bold">Donations</h1>
    </div>
    <div v-for="donation in donations" :key="donation.id" class="py-12">
      <div @click='selectdonation(donation.id)'>
      ...
      <img :src='donation.title'  class="float-right max-w-sm h-auto transition-shadow ease-in-out duration-600 shadow-none hover:shadow-xl" />
    </div>
  ...
  <script>
    ...
    async ngoDetail(){
      const res= await axios.get(this.donations.ngo_list)
      this.ngo=res.data
    }
